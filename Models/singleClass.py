...
import Processing.Evaluator_SingleClass.Evaluator_SingleClass as evaluator_singleClass
import DataPreprocessing.DeepFashionDataHelper.DeepFashionDataHelper as fashionDataset
import DynamicConnections.SingleClass_DynamicPruning.FineTuneModel as singleClass_DynamicPruning
import DynamicLayers.SingleClass_DynamicLayers.FineTuneModel as singleClass_DynamicLayers
...
def main():
    ...
    print("num_attribute_classes is Dynamic")
    ...
    if args.finetune:
        ...
        if args.pruningOrLayers == "1":
            # Dynamic Pruning
            model = singleClass_DynamicPruning(original_model, args.arch)
        else:
            # Dynamic Layers
            model = singleClass_DynamicLayers(original_model, args.arch)
    else:
    ...
    # Data loading code
    transformed_dataset = fashionDataset(args.data,
                                         args.annotation,
                                         args.eval,
    ...
    validation_dataset = fashionDataset(args.data,
                                         args.annotation,
                                         args.eval,
    ...
    if args.evaluate:
        evaluator_singleClass.validate(val_loader, model, criterion, args.print_freq)
        return

    for epoch in range(args.start_epoch, args.epochs):
        evaluator_singleClass.adjust_learning_rate(optimizer, epoch,args.lr)

        # train for one epoch
        evaluator_singleClass.train(train_loader, model, criterion, optimizer, epoch, args.print_freq)

        # evaluate on validation set
        prec1_category, prec1_attributes, recall1_attributes = evaluator_singleClass.validate(val_loader, model, criterion, args.print_freq)

        # Can be done for attributes as well  = Shathat
        # remember best prec@1 and save checkpoint
        is_best = prec1_attributes > best_prec1
        best_prec1 = max(prec1_attributes, best_prec1)
        evaluator_singleClass.save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
